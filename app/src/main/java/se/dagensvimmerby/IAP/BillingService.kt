            ...
            purchases@ for (purchase in purchasesList) {
                if (purchase.purchaseState == Purchase.PurchaseState.PURCHASED && purchase.sku.isSkuReady()) {
                    ...
                    if (Looper.myLooper() == Looper.getMainLooper()) {
                        Log.d("ThreadCheck", "This is mainThread")
                    } else {
                        Log.d("ThreadCheck", "This is background thread.")
                    }
                    ...
                    if (!purchase.isAcknowledged) {
                        MyParseInstallation.validatePurchase(purchase.packageName, purchase.sku, purchase.purchaseToken,
                                object: ParseTools.DoneCallback {
                            override fun done() {
                                val acknowledgePurchaseParams = AcknowledgePurchaseParams.newBuilder()
                                        .setPurchaseToken(purchase.purchaseToken).build()
                                mBillingClient.acknowledgePurchase(acknowledgePurchaseParams, object: AcknowledgePurchaseResponseListener {
                            ...
                            override fun error(e: ParseException?) {
                                log("processPurchases: validation failed")
                            }
                        })
                    }
                } else {
